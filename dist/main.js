...
const createStoreItems = function(arr) {
  let items = "";
  arr.forEach((item) => {
    items = items+`<li class="blog-articles-item">
...
const loadProducts = pageNumber => {
  ...
  runSpinner();
  setTimeout(() => {
    fetch(`store_jsons/goods_${pageNumber}_part.json`).then(response => response.json()).then(goods => {
      products = goods.concat(products);
